...
import { Invoice, AdAccount, Vendor, BillingCard, Series, PlatformType } from '../../types';
...
function InsightsView({ invoices, adAccounts, vendors, cards = [], series = [], selectedAccId, onSelectAccId }: InsightsViewProps) {
  // Top level filters
          ...
          <div className="p-4 space-y-3">
            <div>
              ...
              <div className="rounded-xl border border-slate-200/70 dark:border-slate-800 overflow-hidden">
                {[
                  { label: 'Approved', count: filteredInsightsInvoices.filter(inv => (inv.approvalStatus || inv.paymentVerificationStatus || 'Approved') === 'Approved').length, tone: 'emerald' as const, dot: '#10b981' },
                  { label: 'Declined', count: filteredInsightsInvoices.filter(inv => { const a = inv.approvalStatus || inv.paymentVerificationStatus || 'Approved'; return a === 'Rejected' || a === 'Declined'; }).length, tone: 'rose' as const, dot: '#f43f5e' },
                  { label: 'Pending', count: filteredInsightsInvoices.filter(inv => { const a = inv.approvalStatus || inv.paymentVerificationStatus || 'Approved'; return a === 'Pending'; }).length, tone: 'amber' as const, dot: '#f59e0b' },
                ].map((row, idx) => {
                  const total = filteredInsightsInvoices.length || 1;
                  const pct = Math.round((row.count / total) * 100);
                  const isApproved = row.tone === 'emerald';
