...
import { memo, useState, type FormEvent, type MouseEvent as ReactMouseEvent } from 'react';
import { Plus, FileEdit, DollarSign, User, Check } from 'lucide-react';
import { Vendor, PlatformType } from '../../types';
...
function VendorsView({ vendors, onAddVendor, onUpdateVendor, paymentMethods }: VendorsViewProps) {
  const [search, setSearch] = useState('');
  const [selectedVendorId, setSelectedVendorId] = useState(vendors[0]?.id || '');
  const [showModal, setShowModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [showPayModal, setShowPayModal] = useState(false);
  const [payVendorData, setPayVendorData] = useState<Vendor | null>(null);
  ...
  const handlePaySubmit = (e: FormEvent | ReactMouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4" id="form-add-vendor">
          ...
          <div>
            ...
            <select value={plat} onChange={(e) => setPlat(e.target.value as PlatformType)} className="w-full text-xs p-2 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 dark:text-white font-medium">
          ...
          <div>
            ...
            <select value={status} onChange={(e) => setStatus(e.target.value as Vendor['status'])} className="w-full text-xs p-2 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 dark:text-white font-medium">
        ...
        <form onSubmit={handleEditSubmit} className="space-y-4" id="form-edit-vendor">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => editVendorData && setEditVendorData({ ...editVendorData, platform: e.target.value as PlatformType })}
              className="w-full text-xs p-2 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 dark:text-white font-medium"
            >
              <option value="Facebook">Facebook</option>
              <option value="TikTok">TikTok</option>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => editVendorData && setEditVendorData({ ...editVendorData, status: e.target.value as Vendor['status'] })}
              className="w-full text-xs p-2 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-200 dark:border-slate-800 dark:text-white font-medium"
            >
              <option value="Active">Active</option>
